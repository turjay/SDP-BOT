...
def test_stochastic_backtest():
    balance_history, _ = run_backtest(stochastic_trade_signal, 'Stochastic Oscillator')
    assert len(balance_history) > 0
    assert balance_history[-1] > 0

def test_atr_backtest():
    balance_history, _ = run_backtest(atr_trade_signal, 'ATR')
    assert len(balance_history) > 0
    assert balance_history[-1] > 0
